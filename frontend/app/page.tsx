...
import { currentUsers, initialRecentPosts, initialNotices } from "@/lib/data"
...
export default function DogrunsApp() {
  ...
  const unreadNoticesCount = useMemo(() => notices.filter((notice) => !notice.read).length, [notices])
  ...
  const handleRegistrationSubmit = useCallback(
    (e: FormEvent) => {
      ...
      console.log("Vaccination Certificate File:", vaccinationCertificateFile)
      setUserStatus(UserStatus.RegistrationPending)
    },
    [vaccinationCertificateFile, setUserStatus],
  )
  ...
  const handleLogout = useCallback(() => {
    setUserStatus(UserStatus.ApprovedNeedsLogin)
  ...
  const handleAddDogRegistrationSubmit = useCallback(
    (e: FormEvent) => {
      ...
      console.log("New Dog Vaccination Certificate File:", newDogVaccinationCertificateFile)
      setNewDogRegistrationStatus(NewDogRegistrationStatus.Pending)
    },
    [newDogVaccinationCertificateFile, setNewDogRegistrationStatus],
  )

  const handleApproveNewDog = useCallback(() => {
    const newDogId = Date.now()
    const newDog: DogProfile = {
      id: newDogId,
      name: newDogName,
      breed: newDogBreed,
      weight: `${newDogWeight}kg`,
      owner: "田中さん",
      time: "新規登録",
    ...
    setNewDogRegistrationStatus(NewDogRegistrationStatus.Approved)
    ...
    setTimeout(() => {
      ...
      setNewDogVaccinationCertificateFile(null)
    }, 1500)
  }, [
    newDogName,
    newDogBreed,
